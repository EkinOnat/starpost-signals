...
describe("HTTP and SSE API", () => {
  ...
  it("stores only allowlisted aggregate product events", async () => { const store = await storeFixture(); const api = createIndexerServer({ store, allowedOrigins: [], rateLimitPerMinute: 20, health: () => ({ ready: true }) }); await new Promise<void>((resolve) => api.server.listen(0, "127.0.0.1", resolve)); cleanups.push(() => new Promise<void>((resolve) => api.server.close(() => resolve()))); const port = (api.server.address() as AddressInfo).port; const base = `http://127.0.0.1:${port}`; expect((await fetch(`${base}/api/v1/telemetry`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ event: "onboarding_action_confirmed", wallet: "must-not-be-stored" }) })).status).toBe(204); expect(await fetch(`${base}/api/v1/metrics`).then((response) => response.json())).toMatchObject({ events: { onboarding_action_confirmed: { count: 1 } } }); expect(store.snapshot().metrics).not.toHaveProperty("wallet"); expect((await fetch(`${base}/api/v1/telemetry`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ event: "feedback_submitted" }) })).status).toBe(400); expect((await fetch(`${base}/api/v1/telemetry`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ event: "full_wallet_address" }) })).status).toBe(400); });
